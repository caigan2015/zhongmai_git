...
class UserController extends CommonController {
    ...
    public function ajax_index(){
        ...
        if($pages<$page){
            ...
            $list = $applyDB->where($where)->limit($pageset, $pagenum)->order('ctime desc')->select();
            foreach($list as $ka=>$va){
                $p = M('Products')->where(['id'=>$va['pid']])->find();
                $list[$ka]['title']=$p['title'];
                $list[$ka]['money']=$p['money'];
        ...
        foreach ($list as $key=>$val){
            $str.= '<li>
                    <div class="data"><a href="'.U('User/order',array('id'=>$val['id'])).'"><span class="staus red">'.$val['state'].'</span>申请时间：'.$val['ctime'].' </a></div>
                    <div class="left">
                        <a href="javascript:void(0);"><img src="http://'.SITE_URL.'/Public/upload/product/s_'.$val['img'].'" class="img" /></a>
                    </div>
                    <div class="right">
                        <p class="title">'.$val['title'].'</p>
                        <p class="des"><span class="key">厂商指导价：</span><span class="name">'.$val['money'].'万</span></p>
                        <p class="des"><span class="key">月　　 供：</span><span class="name">'.$val['price'].'元</span></p>
                    </div>
                </li>';
        }
